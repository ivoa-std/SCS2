...
Also, severals years after ConeSearch-1, the IVOA defined the
...
SCS2 services are required to fail a request passes a parameter they do
...
parameter should have a UCD of \ucd{pos.outline}.
...
Clients must pass exactly one position-constraining pararameter in
a request, i.e., either RA, DEC, SR, or (on supporting services) POS or
UPLOAD.  If the do, the
service must respond with a 400 Bad Request HTTP status and a DALI error
...
one coming with the specification.  For now, it is unchanged over what
...
\item[Occurrence] optional
\item[Comment]
                        This, in effect, is a hard limit on how far clients
                        can raise MAXREC.
...
\item[Meaning]
                        True if the service will vary the column set
...
\item[Occurrence] optional
\item[Comment]
                   Services that can search in more than one table
...
assuming in January 2026; we are aware the initial WD has already come
later than that.}
\label{fig:timeline}
\end{figure}
...
It furthermore already has the stat element that is expected to
